...
const trustedSourcesList = Object.keys(trustedSourcesDict);

var untrustedSourcesDict = {
...
const untrustedSourcesList = Object.keys(untrustedSourcesDict);
...
const satireList = Object.keys(satireDict);
...
const callback = function(mutationsList, observer) {
    ...
    for(let mutation of mutationsList) {
        // If addedNodes property has one or more nodes
        if(mutation.addedNodes.length) {
            const posts = document.querySelectorAll('article');
            const singleElement = document.querySelector('main');
            posts.forEach(post => {
                // Check if our custom div already exists to avoid duplicates
                if(!post.querySelector('.save-content-div') && !post.querySelector('.unsave-content-div') && !post.querySelector('.satire-content-div') && !post.querySelector('.notChecked-content-div')){
                    // read name of the account
                    var name;
                    try {
                        name = post.firstChild.firstChild.firstChild.children[1].firstChild.firstChild.firstChild.firstChild.firstChild.textContent;
                    } catch (e) {
                        console.log('FIRST MATCH FAILED: ', e);
                    }
                    if (name === undefined) {
                        try {
                            name = post.firstChild.children[1].firstChild.firstChild.firstChild.firstChild.firstChild.children[1].firstChild.firstChild.firstChild.firstChild.firstChild.textContent;
                            console.log('SECOND MATCH: ', name);
                        } catch (e) {
                            console.log('SECOND MATCH FAILED: ', e);
                        }
                    } else {
                        console.log('FIRST MATCH: ', name);
                    }
                    if (name === undefined) {
                        name = post.querySelector('a[role="link"]').textContent;
                        console.log('THIRD MATCH: ', name);
                    }
                    if (name === undefined) {
                        name = post.querySelector('span').textContent;
                        console.log('FOURTH MATCH: ', name);
                    }


                    console.log('FINAL SELECTION: ', name);
                    
                    // Vertrauenswürdig div
                    const saveDiv = document.createElement('div');
                    saveDiv.textContent = ' Eher Vertrauenswürdig';
                    saveDiv.className = 'save-content-div';
                    saveDiv.style.backgroundColor = 'green';

                    // nicht Vertrauenswürdig div
                    const unsaveDiv = document.createElement('div');
                    unsaveDiv.textContent = ' Eher nicht Vertrauenswürdig';
                    unsaveDiv.className = 'unsave-content-div';
                    unsaveDiv.style.backgroundColor = 'orange';
                    unsaveDiv.style.color = 'black';

                    // satire div
                    const satireDiv = document.createElement('div');
                    satireDiv.textContent = ' Satire';
                    satireDiv.className = 'satire-content-div';
                    satireDiv.style.backgroundColor = 'blue';

                    // satire div
                    const notCheckedDiv = document.createElement('div');
                    notCheckedDiv.textContent = ' Nicht geprüft';
                    notCheckedDiv.className = 'notChecked-content-div';
                    notCheckedDiv.style.backgroundColor = 'yellow';
                    notCheckedDiv.style.color = 'black';

                    //if (trustedSourcesList.includes(name)) {  //if (name.includes('pubity')) {

                    //if (name.includes('pubity')) {
                    //if (trustedSourcesList.some(elem => name.includes(elem))) {  // name enthält ausschnitte der trusted list 
                    
                    if (trustedSourcesList.includes(name)) {  // trusted list enthält genau den namen der instagram seite 

                        console.log('Vertrauenswürdig', name);
                        post.prepend(saveDiv);
                    } else if (untrustedSourcesList.includes(name)) {
                        console.log('nicht Vertrauenswürdig', name);
                        post.prepend(unsaveDiv);
                    } else if (satireList.includes(name)) {
                        console.log('satiere', name);
                        post.prepend(satireDiv);
                    } else {
                        console.log('nicht geprüft', name);
                        post.prepend(notCheckedDiv);
                    }
                    
                    //console.log('FIRST CASE: ', name);
                }
            });
            if (posts.length === 0) {
                try{
                if(!singleElement.querySelector('.save-content-div') && !singleElement.querySelector('.unsave-content-div') && !singleElement.querySelector('.satire-content-div') && !singleElement.querySelector('.notChecked-content-div')){
                    console.log('no articles found, trying to find stadnard content');
                    const singleElementName = singleElement.firstChild.firstChild.firstChild.children[1].firstChild.firstChild.firstChild.children[1].firstChild.firstChild.firstChild.firstChild.firstChild;
                    if (singleElementName) {
                        var name = singleElementName.textContent;
                        

                        // SAVE div
                        const saveSingleDiv = document.createElement('div');
                        saveSingleDiv.textContent = ' Eher Vertrauenswürdig';
                        saveSingleDiv.className = 'save-content-div';
                        saveSingleDiv.style.backgroundColor = 'green';

                        // UNSAVE div
                        const unsaveSingleDiv = document.createElement('div');
                        unsaveSingleDiv.textContent = ' Eher nicht Vertrauenswürdig';
                        unsaveSingleDiv.className = 'unsave-content-div';
                        unsaveSingleDiv.style.backgroundColor = 'orange';
                        unsaveSingleDiv.style.color = 'black';

                        // satire div
                        const satireSingleDiv = document.createElement('div');
                        satireSingleDiv.textContent = ' Satire';
                        satireSingleDiv.className = 'satire-content-div';
                        satireSingleDiv.style.backgroundColor = 'blue';

                        // satire div
                        const notCheckedSingleDiv = document.createElement('div');
                        notCheckedSingleDiv.textContent = ' Nicht geprüft';
                        notCheckedSingleDiv.className = 'notChecked-content-div';
                        notCheckedSingleDiv.style.backgroundColor = 'yellow';
                        notCheckedSingleDiv.style.color = 'black';

                        console.log('trying to add a single+++++++++++++++++++++++++++');
                        //if (name.includes('pubity')) {
                        //if (trustedSourcesList.some(elem => name.includes(elem))) {  // name enthält ausschnitte der trusted list 
                        if (trustedSourcesList.includes(name)) {  // trusted list enthält genau den namen der instagram seite 
                            console.log('SINGLE: Vertrauenswürdig', name);
                            singleElement.prepend(saveSingleDiv);
                        } else if (untrustedSourcesList.includes(name)) {
                            console.log('SINGLE: nicht Vertrauenswürdig', name);
                            singleElement.prepend(unsaveSingleDiv);
                        } else if (satireList.includes(name)) {
                            console.log('SINGLE: satiere', name);
                            singleElement.prepend(satireSingleDiv);
                        } else {
                            console.log('SINGLE: nicht geprüft', name);
                            singleElement.prepend(notCheckedSingleDiv);
                        }
                    } 
                }
            }catch (e) {
                console.log('SINGLE ELEMENT ERROR: ', e);
        } 
...
const singleElement = document.querySelector('main');
const unsaveDiv = document.createElement('div');
unsaveDiv.textContent = 'nicht Vertrauenswürdig';
unsaveDiv.className = 'unsave-content-div';
unsaveDiv.style.backgroundColor = 'red';
singleElement.prepend(unsaveDiv);
*/